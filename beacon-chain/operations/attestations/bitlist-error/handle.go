...
func (h *BitlistErrorHandler) Handle(id attestation.Id, att ethpb.Att, seenBitlist bitfield.Bitlist) {
	...
	seenFilename := os.TempDir() + "/" + hexutil.Encode([]byte(id.String())) + "-seen-" + strconv.FormatUint(count, 10) + ".json"
	...
	log.Debugf(
		...
		att.GetAggregationBits().Len(),
		seenBitlist.Len(),
		attFilename,
		dumpFilename,
	)

	jsonSeen, marshalErr := json.Marshal(seenBitlist)
	if marshalErr != nil {
		log.WithError(marshalErr).Debug("Could not marshal seen bitlist into JSON")
	}
	if writeErr := file.WriteFile(seenFilename, jsonSeen); writeErr != nil {
		log.WithError(writeErr).Debug("Could not write seen bitlist JSON to file")
	}
