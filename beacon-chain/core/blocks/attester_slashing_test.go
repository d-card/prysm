...
func TestProcessAttesterSlashings_DataNotSlashable(t *testing.T) {
	...
	_, _, err = blocks.ProcessAttesterSlashings(context.Background(), beaconState, ss, v.SlashValidator, v.ExitInformation(beaconState))
...
func TestProcessAttesterSlashings_AppliesCorrectStatus(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			newState, _, err := blocks.ProcessAttesterSlashings(context.Background(), tc.st, []ethpb.AttSlashing{tc.slashing}, v.SlashValidator, v.ExitInformation(tc.st))
			require.NoError(t, err)
