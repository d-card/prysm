...
func TestProcessVoluntaryExits_NotActiveLongEnoughToExit(t *testing.T) {
	...
	_, _, err = blocks.ProcessVoluntaryExits(context.Background(), state, b.Block.Body.VoluntaryExits, validators.ExitInformation(state))
...
func TestProcessVoluntaryExits_AppliesCorrectStatus(t *testing.T) {
	...
	newState, _, err := blocks.ProcessVoluntaryExits(context.Background(), state, b.Block.Body.VoluntaryExits, validators.ExitInformation(state))
